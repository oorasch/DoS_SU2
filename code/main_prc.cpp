...
void do_restricted_MC(const size_t& IntNum, const double& Qmin, const double& Qmax, vector<int>* neib,
...
string FOLDER_IN = "../prc/start_configs/Nt"+to_string(params::Nt)+"_Ns"+
                    to_string(params::Ns)+"_betastart"+to_string((int)(params::beta_start*10))+"/";

string FOLDER_OUT = "../prc/stats/Nt"+to_string(params::Nt)+"_Ns"+
...
int main(){
  ...
  vector<double> Q_steps = load_Qsteps_data(FOLDER_IN+FILENAME);


  //loop over Q_steps-1 intervals
  // #pragma omp parallel for simd firstprivate(U_vec)
  for(size_t k = 0; k < Q_steps.size()-1; k++)
  {
    double Q = 0.0;

    //get configuration for this interval
    FILENAME = "start_config_Int"+to_string(k)+".dat";
    read_config(FOLDER_IN+FILENAME, U_vec);
    Q = meas_topo_clov(neib.data(),U_vec.data());

    cout << Q << endl;

    do_restricted_MC(k, Q_steps.at(k), Q_steps.at(k+1), neib.data(), U_vec.data(), Q);
...
void do_restricted_MC(const size_t& IntNum, const double& Qmin, const double& Qmax, vector<int>* neib,
                      ...
                      double& Q)
{
  //initialize lambda vals
  vector<double> lambda_vals = lambda_vals_init_prc();

  //open file for restr. expectation values
  string FILENAME = to_string(IntNum)+"_RE_Q_lambda_stats.dat";
  ...
  for(size_t l = 0; l < lambda_vals.size(); l++)
  {
    ...
    outfile << lambda_vals.at(l) << " " << mean << " " << err << endl;
